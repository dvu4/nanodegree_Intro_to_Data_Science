...
def custom_heuristic(file_path):
    ...
    for passenger_index, passenger in df.iterrows():
        passenger_id = passenger['PassengerId']
        if (passenger['Pclass'] == 1 and passenger['Age'] <= 18) or passenger['Sex'] == 'female'  or passenger['Parch'] >6:
            predictions[passenger_id] = 1
        else:
            predictions[passenger_id] = 0
    ...
    print pandas.crosstab(df['Sex'], df['Survived'])
    print pandas.crosstab(df['Age'], df['Survived'])
    print pandas.crosstab(df['Pclass'], df['Survived'])
    print pandas.crosstab(df['Fare'], df['Survived'])
    print pandas.crosstab(df['Parch'], df['Survived'])
    print pandas.crosstab(df['Cabin'], df['Survived'])
    print pandas.crosstab(df['Embarked'], df['Survived'])
    return predictions
